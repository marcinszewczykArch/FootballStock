...
trait PlayerService[F[_]] {
  def searchByName(playerName: String): F[List[PlayerSimple]]
...
object PlayerService {
  ...
  def impl[F[_]: Sync](client: TransfermarktClient[F]) = new PlayerService[F] {
    ...
    override def searchByName(
      playerName: String
    ): F[List[PlayerSimple]] = client
      .searchByName(playerName)
      .map(_.map(_.map(toPlayerSimple)).getOrElse(Nil))
    ...
    override def getMarketValueByPlayerId(id: Int): F[Either[GameException, MarketValue]] =
      client.getMarketValueByPlayerId(id).map(_.map(toMarketValue)).map {
        case Some(marketValue) => Right(marketValue)
        case None              => Left(PlayerMarketValueNotFoundException(id))
      }
