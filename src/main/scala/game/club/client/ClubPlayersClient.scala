...
trait ClubPlayersClient[F[_]] {
  def fetchRawClubPlayersById(id: ClubId): F[Either[GameException, Json]]
}
...
object ClubPlayersClient {
  ...
  def impl[F[_]: Sync: MonadThrow](config: ClubPlayersClientConfig) = new ClubPlayersClient[F] {
    ...
    override def fetchRawClubPlayersById(id: ClubId): F[Either[GameException, Json]] = Applicative[F].pure(for {
