...
import game.modules.player.service.PlayerMapper._
...
trait PlayerService[F[_]] {
  ...
  def refreshPlayerProfileInMemory(playerId: PlayerId): F[ErrorOr[Unit]]
  def getAllPlayerProfilesJsonDirectlyFromMemory(): F[Map[PlayerId, Json]]
}
...
object PlayerService {
  ...
  def impl[F[_]: Sync: LoggerFactory](
    ...
    override def refreshPlayerProfileInMemory(playerId: PlayerId): F[ErrorOr[Unit]] = (for {
    ...
    override def getAllPlayerProfilesJsonDirectlyFromMemory(): F[Map[PlayerId, Json]] =
      playerProfileClientMemory.getAll()
  }
