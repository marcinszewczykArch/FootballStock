package game.modules.state.service
...
import game.modules.event.Event.SYSTEM_USER_NAME
import game.modules.state.domain.{Shares, User, UserGameState}
  ...
  def impl[F[_]: Sync: LoggerFactory](userGameStateMemory: UserGameStateMemory[F]) = new UserGameStateService[F] {
    ...
    override def calculateSharesAfterBuy(
      ...
      sharesInPortfolio.sum + sharesToBuy <= 100 match {
        case true  => Right(sharesInPortfolio |+| Shares(sharesToBuy, currentPlayerMarketValue, timeProvider.getCurrentTimestamp, buyMinutesPlayed, minutesPlayedLastSeen, dividend))
